...
module Com.Mysql.Cj.Mysqlx.Protobuf.FetchDoneMoreResultsets (FetchDoneMoreResultsets(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor FetchDoneMoreResultsets where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".Mysqlx.Resultset.FetchDoneMoreResultsets\", haskellPrefix = [], parentModule = [MName \"Com\",MName \"Mysql\",MName \"Cj\",MName \"Mysqlx\",MName \"Protobuf\"], baseName = MName \"FetchDoneMoreResultsets\"}, descFilePath = [\"Com\",\"Mysql\",\"Cj\",\"Mysqlx\",\"Protobuf\",\"FetchDoneMoreResultsets.hs\"], isGroup = False, fields = fromList [], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"
